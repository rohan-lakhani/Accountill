...
const PaymentHistory = ({ paymentRecords }) => {
  ...
  return (
    <div className='w-3/5 mx-auto mt-6'>
            ...
            <Badge badgeContent={paymentRecords?.length} color="primary" sx={{marginY:"15px", marginX:"17px", bgcolor:"red"}} classes={{badge: classes.font1}}></Badge>
            </AccordionSummary>
            <AccordionDetails>
                        ...
                        ?.map((record,index) => {
                          return (
                            <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                              <TableCell scope='row'>{moment(record.datePaid).format("YYYY-MM-DD")}</TableCell>
                              <TableCell align='left'>{toCommas(record.amountPaid.toFixed(2))}</TableCell>
